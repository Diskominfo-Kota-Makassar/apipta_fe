...
import { getPenugasanFromAPI, getAllFileAuditFromAPI, getUsersFromAPI } from 'src/utils/api';
...
export default function LaporanPage() {
  ...
  const [penugasanTerpilih, setPenugasanTerpilih] = useState([]);
  ...
  const handlePenugasanFromAPI = useCallback(async () => {
    try {
      ...
      const pngs = penugasan.data.filter((png) => {
        const valueFile = valueDraftNaskahFromAPI.find(
          (file) => file.penugasan_id === png.id.toString()
        );
        return valueFile && valueFile.files.length === 8;
      });
      setAllPenugasan(pngs);
    } catch (error) {
      console.error('Error fetching penugasan data:', error);
      // Handle error appropriately
    }
  }, [valueDraftNaskahFromAPI]);

  const handleUsersFromAPI = useCallback(async () => {
    const userso = await getUsersFromAPI();
    const usersObject = await userso.data;
  ...
  useEffect(() => {
    ...
    handleUsersFromAPI();
  }, [handlePenugasanFromAPI, handleUsersFromAPI]);
            ...
            <Table sx={{ minWidth: 800 }}>
              <UserTableHead
                ...
                headLabel={[
                  ...
                  // { key: 'No. LHP', id: 'No. LHP', label: 'No. LHP' },
                  { key: 'uraian', id: 'uraian', label: 'URAIAN.ST' },
                  { key: 'objek_pengawasan', id: 'objek_pengawasan', label: 'Objek Pengawasan' },
                  { id: '', label: 'AKSI' },
                ]}
              />
              <TableBody>
